...
- `js/qr-scanner.js` — openQrScanner(), closeQrScanner() — escanea QR con librería qr-scanner (CDN)

## PWA
- manifest.json: start_url "./" (NO "./index.html" — Cloudflare redirige esa URL)
- sw.js: VERSION='v38', dos cachés CACHE_SHELL + CACHE_RUNTIME, stale-while-revalidate para fonts
...
- Campo persistente `mant` en inventario. `HEADERS_INV` en `appscript.txt`: `id|ref|aula|mod|item|qty|min|cat|loc|est|util|fecha|mant|mantFecha|mantNota|mantResp|mantEstado|mantSolicitante|mantSolicitanteEmail|foto|obs`.
...
## Escaneo QR (implementado 2026-05-06)
- Librería `qr-scanner` v1.4.2 desde CDN (en `index.html` línea 840 sin `defer` para carga síncrona)
- Botón 🔍 en topbar (buscador global) abre modal `#mQrScanner` vía `openQrScanner()`
- `js/qr-scanner.js`: 
  - `openQrScanner()` inicia la cámara usando getUserMedia y QrScanner library
  - Try-catch captura errores de inicialización y muestra en modal si librería no carga o cámara no disponible
  - Detecta patrones `item/<id>` en datos QR y llama `openItemRoute(id)` para abrir el ítem
  - `closeQrScanner()` para la cámara y cierra el modal
- Modal muestra video en vivo y detecta QR automáticamente; funciona en tablets/móviles si hay acceso a cámara
- Para tablets iOS/iPad con restricciones de getUserMedia, considerar fallback a file input con `capture="environment"`
