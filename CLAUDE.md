...
- Campo persistente `mant` en inventario. `HEADERS_INV` en `appscript.txt`: `id|ref|aula|mod|item|qty|min|cat|loc|est|util|fecha|mant|mantFecha|mantNota|mantResp|mantEstado|foto|obs`.
...
- Modal de ítem: checkbox `#f_mant` "Necesita mantenimiento o reparación"; detalles en `#maintFields`: `mantFecha`, `mantEstado` (`Pendiente`, `En reparación`, `Resuelto`), `mantResp`, `mantNota`.
- `saveItem()` guarda `mant:'1'` o vacío y los detalles de la cola. `needsMaintenance()` excluye `mantEstado === 'Resuelto'`, pero conserva los datos históricos si el ítem sigue marcado.
